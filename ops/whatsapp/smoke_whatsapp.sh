...
# ── 2) /wa-accounts requires auth (expect 401, not 404) ──────────────────────
TMP="$(mktemp)"
...
elif echo "$BODY" | grep -qi 'accounts\|data'; then
  pass "/api/wa-accounts returns 200 (open endpoint)"
else
  fail "/api/wa-accounts expected 401/403/200, got $CODE body=$BODY"
fi

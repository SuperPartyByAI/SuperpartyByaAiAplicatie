...
REPORT_PATH = os.getenv("REPORT_PATH", "/tmp/report_supabase_structure.md")
...
print("Running Supabase Rules Unit Tests...")
ok, out = run_cmd("npm test --prefix monitoring/supabase/checks/tests")
if ok:
    ...
    log_result("Rules Emulator Tests", "FAIL", "Security rules unit tests failed. Regressions detected.", 
               "Check `monitoring/supabase/checks/tests` and update `database.rules`.", "CRITICAL", out)

# 2. Integration Checks (Staging/Production probe via bash)
print("Running Integration Checks...")
ok, out = run_cmd("bash monitoring/supabase/checks/integration_checks.sh")
if ok:
    ...
    log_result("Integration Probes", "FAIL", "One or more integration checks failed (e.g., server-field writes bypassed).", 
               "Review integration_checks.sh output and tighten Database security rules.", "CRITICAL", out)
...
if not ok:
    log_result("Backup Export Existence", "FAIL", f"Could not list bucket {BACKUP_BUCKET} or it is empty.", 
               "Ensure Supabase Scheduled Export is running or check IAM permissions.", "CRITICAL", out)
else:
...
with open(REPORT_PATH, "w") as f:
    f.write("# Supabase Structure & Security Audit Report\n\n")

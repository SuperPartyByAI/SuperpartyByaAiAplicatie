...
function convertRolesToRolesBySlot(roles, eventShortId) {
  ...
  roles.forEach((role, index) => {
    ...
    rolesBySlot[slotKey] = {
      ...
      pendingCode:
        role.pendingCode ||
        role['Cod în așteptare'] ||
        role['Cod in asteptare'] ||
        role.codInAsteptare ||
        null,
    ...
    const payment = {
      status:
        incasare.stare === 'NEINCASAT'
          ? 'UNPAID'
          : incasare.stare === 'INCASAT'
            ? 'PAID'
            : incasare.stare === 'ANULAT'
              ? 'CANCELLED'
  ...
  try {
    ...
    if (stats.examples.length > 0) {
      ...
      stats.examples.forEach((ex, i) => {
        ...
        console.log(
          `  Before: schemaVersion=${ex.before.schemaVersion}, roles=${ex.before.rolesCount}, spaceKeys=${ex.before.hasSpaceKeys}`
        );
        console.log(
          `  After:  schemaVersion=${ex.after.schemaVersion}, eventShortId=${ex.after.eventShortId}`
        );

...
if (!initialized) {
  ...
  console.log(
    '  1. Go to: https://console.firebase.google.com/project/superparty-frontend/settings/serviceaccounts/adminsdk'
  );
...
async function checkCollection(collectionName, limit = 5) {
  ...
  try {
    ...
    snapshot.forEach((doc, index) => {
      ...
      console.log(
        `      Fields (${keys.length}): ${keys.slice(0, 8).join(', ')}${keys.length > 8 ? '...' : ''}`
      );
      ...
      if (data.rolesBySlot) {
        ...
        console.log(
          `      🎭 rolesBySlot: ${slots.length} roles [${slots.slice(0, 3).join(', ')}${slots.length > 3 ? '...' : ''}]`
        );
      }

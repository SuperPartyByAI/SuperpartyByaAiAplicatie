...
Problema **nu** e în UI-ul Inbox, ci în **pipeline-ul de ingestie**: conversațiile apar în aplicație doar dacă backend-ul WhatsApp a scris deja `threads` / `messages` în Database (prin **history sync** sau **backfill**).
...
1. **Backend (Hetzner)** – ingestie:
   - **History sync:** la pairing/re-pair, `messaging-history.set` → backend creează **thread placeholders** din `chats` (pentru toate conversațiile), apoi ingestează mesajele în Database. Inbox arată conversațiile; backfill le poate umple.
...
2. **Database** – sursa de adevăr pentru listă:
...
3. **Flutter (Inbox)** – citire:
   ...
   - Pentru fiecare `accountId` permis, **abonează la Database** `threads` (stream), nu la `getThreads` API.
   - Dacă nu există date în Database pentru acel `accountId`, vezi **0 threads** chiar dacă ai „3–4 conturi conectate”.

**Concluzie:** Lista din Inbox se bazează pe **Database**. Fără ingestie (history sync / backfill / realtime) → Database gol pentru acel account → 0 conversații.

---

## Thread-uri `__[object Object]` / `__[obiect Obiect]` și „deduplicare”

**Simptom:** În Database apar thread-uri cu id de forma  
...
**Curățare (opțional):** Thread-urile invalide existente (`*__[object Object]`, `*__[obiect Obiect]`) pot fi șterse manual din Database (colecția `threads`). Subcolecțiile `messages` se șterg odată cu thread-ul. „Deduplicare” / `inboundDedupe` e folosită doar pentru deduplicare; mesajele reale sunt în `threads/{threadId}/messages`.
...
**Simptom:** Backend-ul nu procesează events (history / messages) → Database nu se umple.

**Verificări rapide:**
- **Health:**  
  `curl -s http://HETZNER_IP:8080/health | jq`  
  → `database: "connected"`, `connected` / `accounts_total` ok.
- **Dashboard:**  
...
- [ ] **Backend:** rulat pe Hetzner; `SESSIONS_PATH` corect; `database` connected.
...
**Pipeline trimitere:**
1. App → **whatsappProxySend** (Functions) → creează doc în Database `outbox` (status `queued`) + mesaj optimist în thread → răspunde 200 → UI afișează „Message sent!”.
...
- **ProcessOutbox (Functions) e dezactivat în mod normal:** Trigger-ul Database `processOutbox` e **oprit implicit** (ar trimite la backend fără auth → 401). Livrarea se face **doar** prin worker-ul de outbox din backend. Asigură-te că backend-ul rulează, e ACTIVE și are acces la Database.
- **Backend și Functions folosesc Database diferit:** Worker-ul citește `outbox` din Database. Dacă backend-ul folosește alt proiect/DB decât Functions, nu vede documentele create de proxy.
...
4. **Health:**  
   `curl -s http://HETZNER_IP:8080/health | jq`  
   → `database: "connected"`, conturi `connected`.
...
- **Inbox / filtrare:** `superparty_flutter/lib/screens/whatsapp/staff_inbox_screen.dart` (conturi `status == 'connected'`, Database `threads`), `whatsapp_inbox_screen.dart`.

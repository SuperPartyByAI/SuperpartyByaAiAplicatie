...
3. Sets status to `needs_qr` in Database
...
- Database: `accounts/{accountId}.status = 'needs_qr'`
- Database: `accounts/{accountId}.nextRetryAt = null`
- Database: `accounts/{accountId}.retryCount = 0`
...
- `createConnection blocked: database status=needs_qr` (should appear)

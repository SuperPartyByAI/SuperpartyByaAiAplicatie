...
This runbook documents the **best-effort full conversation sync** feature that ensures WhatsApp message history is persisted in Database:

1. **On pairing/re-pair:** Ingest WhatsApp history sync (chats + messages) into Database
...
4. **Database schema:** Consistent and queryable (`threads/messages` structure)
...
| `WHATSAPP_HISTORY_SYNC_DRY_RUN` | `false` | If `true`, logs sync counts but doesn't write to Database |
...
- **History Sync (`syncFullHistory: true`):** On initial pairing or re-pairing, Baileys provides a history sync event (`messaging-history.set`) containing all chats and messages. These are ingested into Database automatically.
...
- **Realtime (primary):** `messages.upsert` → `saveMessageToDatabase` → Database `threads/{id}`, `threads/{id}/messages/{id}`. Logs: `[realtime] accountId=... remoteJid=... msg=... ts=... type=... writeOK=true|false`. Diagnostics: `accounts/{id}.lastRealtimeIngestAt`, `lastRealtimeMessageAt`, `lastRealtimeError`.
...
- **Realtime diagnostics:** `lastRealtimeIngestAt` (serverTimestamp, updated on each messages.upsert), `lastRealtimeMessageAt` (timestamp from message), `lastRealtimeError` (optional, when Database write fails).
  ...
  "messages": [
    {
      ...
      "messageType": "text",
      "tsClient": "ISO8601",
      "tsServer": "DatabaseTimestamp",
      "createdAt": "DatabaseTimestamp",
      "syncedAt": "DatabaseTimestamp",
...
**Note:** Backfill runs asynchronously. Check `accounts/{accountId}.lastBackfillResult` in Database for results.
...
3. **Distributed lock:** Per-account Database collection `whatsapp_backfill_locks/{accountId}` with fields `ownerId`, `expiresAtMs`, `startedAt`. Only one instance runs backfill per account; others skip. Lock is released on completion or expiry.
...
- **Database:** `accounts/{accountId}` → `lastAutoBackfillAt`, `lastAutoBackfillSuccessAt`, `lastAutoBackfillAttemptAt`, `lastAutoBackfillStatus`, lease fields; **realtime:** `lastRealtimeIngestAt`, `lastRealtimeMessageAt`, `lastRealtimeError`; **recent-sync:** `lastRecentSyncAt`, `lastRecentSyncResult`, `recentSyncLeaseUntil`, `recentSyncLeaseHolder`.
...
# Database: accounts/{accountId} → lastRealtimeIngestAt, lastRealtimeMessageAt
```
...
**Database (Console):** `accounts/{accountId}` → `lastRealtimeIngestAt`, `lastRealtimeMessageAt`, `lastRecentSyncAt`, `lastAutoBackfillStatus`.
...
Once `/diag` returns **200**, `mode` is **active**, and `databaseConnected` is **true**, the next step is **not** deploy but **validating** that live ingest and gap-filler/backfill actually bring missing messages (yesterday/today).

#### 1. Confirm live ingest (new message → Database)
...
2. **On the server**, watch logs:
   ...
   ```
3. **In Database**, open `accounts/{accountId}` and check whether:
...
Share the values from **Database** `accounts/{accountId}` for:
...
**Expected:** `HTTP/1.1 200 OK` and JSON with `ready`, `mode`, `instanceId`, `databaseConnected`, `latestRealtime`, `timestamp`. No HTML, no 404.
...
**Goal:** Fix SSH access, pull latest code, restart, and verify `/diag` returns **200 JSON**. Do not change any `whatsappProxy*` Supabase functions.
...
- [ ] `curl -i http://127.0.0.1:8080/diag` on server → **HTTP/1.1 200 OK** and JSON (`ready`, `mode`, `instanceId`, `databaseConnected`, `latestRealtime`, `timestamp`).
...
- [ ] **Local:** `curl -s http://127.0.0.1:8080/diag` → 200 JSON with `ready`, `mode`, `instanceId`, `databaseConnected`, `latestRealtime`. (If EADDRINUSE, see "Troubleshooting: EADDRINUSE" above.)
...
- [ ] **/diag** never throws when Database is down; returns `databaseConnected: false` and `latestRealtime: []`.

---

## Verification in Database Console

### Step 1: Verify History Sync

1. Supabase Console → Database → Data
...
Use **Supabase Console** → Database → `accounts` → `{accountId}` and inspect the fields above. Logs: `📚 [auto-backfill]` for tick summary and per-account start/end.
...
### 1) Auto-backfill status (Database)
...
- **Blocked:** `ok === false` and `errorCode` / `errorMessage` set → fix cause (logs, Database, backend) then retry.

### 2) Live ingestion (not backfill)

**Test:** Send a **new** WhatsApp message in an existing chat. Check if it appears in Database **`threads/{threadId}/messages`** and in **Inbox** within ~10–30 s.

- **Appears:** Live ingestion works. If “yesterday/today” still missing → gap; use **manual backfill** or check **recent-sync** / cooldown.
- **Does not appear:** Problem on backend (WhatsApp connection, sessions, Database writes, **PASSIVE** instance, etc.). Fix that first; backfill alone will not fix live.
...
- **Live ingestion broken:** Connected but no `messages.upsert` / Database writes (sessions, errors, PASSIVE). Fix backend first.
...
1. **Live test:** Send a WhatsApp message → does it show up in Inbox / Database quickly?
2. **Database:** Open `accounts/{accountId}` → note `lastAutoBackfillStatus`, `lastAutoBackfillSuccessAt`, `lastAutoBackfillAttemptAt`, `lastRealtimeIngestAt`, `lastRealtimeMessageAt`.
...
- Check Database connection: `curl .../health | jq .database.status`
...
```
Database `accounts/{accountId}` → `lastRealtimeIngestAt`, `lastRealtimeMessageAt`, `lastRealtimeError`. If `lastRealtimeError` is set, inspect Database write failures (permissions, schema). `GET /diag` shows `latestRealtime` for connected accounts.

**Legacy log pattern (still valid):**
```bash
💾 [accountId] Message saved to Database: {messageId}
```
...
**Check results:**
- Database: `accounts/{accountId}.lastBackfillResult`
...
**Cause:** Inbox reads from Database; if backend hasn’t ingested (history sync, backfill, realtime), there are no threads for those `accountId`s.
...
1. **Helper Functions** (lines ~509-759):
   - `saveMessageToDatabase()` - Idempotent message save
...
- **Message ID as Document ID:** Prevents duplicates (Database `set` with merge)
- **Dedupe Collection:** `inboundDedupe/{accountId}__{messageId}` for inbound messages
- **Batch Writes:** Limited to 500 ops per batch (Database limit)
...
- **Concurrency:** Backfill processes 1-2 threads at a time to avoid overwhelming Database
- **Graceful Shutdown:** Waits up to 30 seconds for pending Database batches

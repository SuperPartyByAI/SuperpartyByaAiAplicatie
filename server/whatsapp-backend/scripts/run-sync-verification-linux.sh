...
ssh -o StrictHostKeyChecking=no "$RESTART_SSH" "bash -lc 'set -euo pipefail; mkdir -p \"$REMOTE_ART_DIR\"; cd \"$REMOTE_BACKEND_DIR\"; export DATABASE_PREFER_REST=true; RUN_RESTART=true node scripts/run-sync-verification.js > \"$REMOTE_JSON\"'"

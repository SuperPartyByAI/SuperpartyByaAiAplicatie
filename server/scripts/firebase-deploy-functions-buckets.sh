#!/usr/bin/env bash
# Deploy Supabase Functions in buckets to avoid CPU quota and isolate failures.
# Run from repo root: ./scripts/supabase-deploy-functions-buckets.sh [bucket]
...
#   1. supabase functions:delete whatsapp --region us-central1 --force
#   2. ./scripts/supabase-deploy-functions-buckets.sh whatsapp-stub

set -e
cd "$(dirname "$0")/.."

BUCKET="${1:-whatsapp-proxy}"
SLEEP_SEC="${SUPABASE_DEPLOY_SLEEP:-4}"
...
case "$BUCKET" in
  whatsapp-proxy)
    ...
    for fn in "${PROXY_FUNCTIONS[@]}"; do
      echo "  -> functions:${fn}"
      supabase deploy --only "functions:${fn}"
      sleep "$SLEEP_SEC"
    done
  ...
  whatsapp-full)
    ...
    supabase deploy --only "functions:whatsappV4,functions:processOutbox"
    ;;
  whatsapp-stub)
    echo "Deploying Gen2 whatsapp stub (410 deprecated)."
    echo "  If Gen1 whatsapp still exists, delete it first. supabase delete often fails; use:"
  ...
  ai)
    ...
    ;;
  staff)
    echo "Deploying staff / database / scheduler..."

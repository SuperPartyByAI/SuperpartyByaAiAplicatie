...
/* ========== Firebase Admin Init ========== */
import { initFirebase, db, syncMessageToFirestore } from "./firebase-sync.js";
...
app.post('/api/voice/incoming', async (req, res) => {
  ...
  try {
    if (From && To && CallSid) {
      await db.collection('calls').doc(CallSid).set({
        ...
        status: 'ringing',
        timestamp: admin.firestore.new Date(),
        noticePlayed: true
      }, { merge: true });
    }
  ...
  if (isOutbound) {
      ...
      const CALLER_ID = '+40373810882'; // Your Twilio Number
...
app.delete('/api/voice/callback/:sid', async (req, res) => {
  try {
    ...
    await db.collection('calls').doc(parentSid).set({
      status: 'canceled',
      endTime: admin.firestore.new Date(),
    }, { merge: true });
...
app.post('/api/voice/status', async (req, res) => {
  ...
  try {
    ...
    await db.collection('calls').doc(CallSid).set({
      ...
      startTime: StartTime ? new Date(StartTime) : admin.firestore.new Date(),
  ...
  try {
    await db.collection('users').doc(userId)
      ...
      .set({ 
        identity, 
        fcmToken, 
        lastSeen: admin.firestore.new Date() 
      }, { merge: true });
  ...
  try {
    await db.runTransaction(async (t) => {
      ...
      if (existingReservedBy && existingReservedBy === uid) {
        t.update(convoRef, {
          reservedBy: uid,
          reservedAt: admin.firestore.new Date(),
...
app.post('/api/user/consent', verifyFirebaseToken, async (req, res) => {
  try {
    ...
    await db.collection('users').doc(uid).set({
      ...
      consentGivenAt: admin.firestore.new Date()
    }, { merge: true });
...
app.post('/api/user/deletion-request', verifyFirebaseToken, async (req, res) => {
  try {
    ...
    const ticketRef = await db.collection('deletion_requests').add({
      ...
      status: 'pending',
      requestedAt: admin.firestore.new Date()
    });
  ...
  try {
    ...
    await db.collection('users').doc(uid).set({
      privacySettings: {
        ...
        updatedAt: admin.firestore.new Date()
      }
    }, { merge: true });

...
1. **Delete old `whatsapp` v1 function via Supabase Console**
...
3. **Click "Ask AI about client"**
   ...
   - Should respond quickly with data from Database
...
**Symptom:**
```
SupabaseFunctionsException: [not-found] Function not found
```
...
**Fix:** Delete manually via Supabase Console (see `MANUAL_STEP_DELETE_OLD_WHATSAPP.md`)

---

### Issue: High latency still present

**Symptom:** Database operations still take 3-4 seconds

**Cause:** Functions not deployed to `europe-west1`

**Verify:**
```bash
supabase functions:list | grep -E "europe-west1"
...
|-----------|-------------|------------|-------------|
| Database read (1 doc) | ~120ms | ~3ms | 40x faster |
| Database write (1 doc) | ~130ms | ~5ms | 26x faster |
...
1. **Code changes:**
   ...
   SupabaseFunctions.instance.httpsCallable(...)
   ```
...
2. Deploy migrated functions: `supabase deploy --only functions:aggregateClientStats,functions:whatsappExtractEventFromThread,functions:clientCrmAsk`
3. Deploy all functions: `supabase deploy --only functions`

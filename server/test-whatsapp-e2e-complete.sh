...
if echo "$HEALTH_RESPONSE" | grep -q '"status":"healthy"'; then
    if echo "$HEALTH_RESPONSE" | grep -q '"database".*"status".*"connected"'; then
        ...
        log_test "1. Railway Health Check" "❌ FAIL" "Database not connected: $HEALTH_RESPONSE"
    fi
else
    log_test "1. Railway Health Check" "❌ FAIL" "Health check failed: $HEALTH_RESPONSE"
fi

# Test 2: Supabase Functions List
echo "=== Checking Supabase Functions ==="
FUNCTIONS_LIST=$(supabase functions:list 2>&1 | grep -i whatsapp || echo "NONE")
if echo "$FUNCTIONS_LIST" | grep -q "whatsappProxyGetAccounts"; then
    log_test "2. Supabase Functions Available" "✅ PASS" "$FUNCTIONS_LIST"
else
    log_test "2. Supabase Functions Available" "❌ FAIL" "WhatsApp functions not found"
fi

# Test 3: Database Rules Check (read-only check)
echo "=== Checking Database Rules ==="
if [ -f "database.rules" ]; then
    if grep -q "NEVER DELETE" database.rules && grep -q "threads" database.rules; then
        log_test "3. Database Rules Protection" "✅ PASS" "Rules file exists with threads/messages protection"
    else
        log_test "3. Database Rules Protection" "⚠️  WARNING" "Rules file exists but may be missing protections"
    fi
else
    log_test "3. Database Rules Protection" "❌ FAIL" "database.rules file not found"
fi
...
echo "1. If old 1st gen function exists, delete it via Supabase Console" >> "$REPORT_FILE"
